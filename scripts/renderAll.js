import { checkZeroStickys } from "./boards.js";
      ...
      const stickyAdd = `
                <div class="sticker" id="${sticky.id}">
                    <div class="button-wrapper">
                        <div class="delete-button" id="${sticky.id}">
                            X
                        </div>
                        <div class="edit-button" id="${sticky.id}">
                        ...
                        <div class="pin-button" id="${sticky.id}">
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  contentWrapper.addEventListener("click", (e) => {
    ...
    if (deleteButton) {
      deleteSticky(e.target.id);
    }

    if (editButton) {
      editSticky(editButton.id);
    }

    if (pinButton) {
      pinSticky(pinButton.id);
    }
  });
});

export function checkLocalStickys() {
  const defaultSticky = [
    {
      content:
        "This your first sticky on the board! Lets get organised and add more stickys to the board.",
